...
lazy_static! {
    static ref INTERRUPT_DESCRIPTOR_TABLE: InterruptDescriptorTable = {
        ...
        idt.bound_range_exceeded.set_handler_fn(handlers::bound_range_exceeded); // Vector 5
        idt.invalid_opcode.set_handler_fn(handlers::invalid_opcode); // Vector 6
        idt.device_not_available.set_handler_fn(handlers::device_not_available); // Vector 7

        idt.invalid_tss.set_handler_fn(handlers::invalid_tss); // Vector 10
        idt.segment_not_present.set_handler_fn(handlers::segment_not_present); // Vector 11
        idt.stack_segment_fault.set_handler_fn(handlers::stack_segment_fault); // Vector 12
        idt.general_protection_fault.set_handler_fn(handlers::general_protection_fault); // Vector 13
        idt.page_fault.set_handler_fn(handlers::page_fault); // Vector 14

        idt.x87_floating_point.set_handler_fn(handlers::x87_floating_point); // Vector 16
        idt.alignment_check.set_handler_fn(handlers::alignment_check); // Vector 17

        idt.simd_floating_point.set_handler_fn(handlers::simd_floating_point); // Vector 19
        idt.virtualization.set_handler_fn(handlers::virtualization_exception); // Vector 20
        idt.cp_protection_exception.set_handler_fn(handlers::control_protection_exception); // Vector 21

        idt.hv_injection_exception.set_handler_fn(handlers::hv_injection_exception); // Vector 28
